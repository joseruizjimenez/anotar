...
describe NotesController do
  ...
  describe 'GET index' do
    it 'should check if the user is logged in' do
      LoginCredential.should_recieve(:find_by_access_token).with('an_access_token')
      get :index, :access_token => 'an_access_token'
    end

    it 'should use the session id to get the session_credential (logged out)' do
      SessionCredential.should_recieve(:find_by_session_id).with('a_session_id')
      get :index, :session_id => 'a_session_id'
    end

    it 'should find all the user_id notes (logged in)' do
      @fake_login_credential = Factory.create(:login_credential, :user_id => 'an_user_id')
      LoginCredential.stub(:find_by_access_token).and_return(@fake_login_credential)
      Note.should_recieve(:find_by_user_id).with(@fake_login_credential.user_id)
      get :index, :access_token => 'an_access_token'
    end

    it 'should find all the session_user_id notes (logged out)' do
      @fake_session_credential = Factory.create(:session_credential, :user_id => 'an_user_id')
      SessionCredential.stub(:find_by_session_id).and_return(@fake_session_credential)
      Note.should_recieve(:find_by_user_id).with(@fake_session_credential.user_id)
      get :index, :session_id => 'a_session_id'
    end
